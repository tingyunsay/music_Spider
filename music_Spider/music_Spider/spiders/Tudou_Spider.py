...
class MusicSpider(scrapy.Spider):
		...
		for v in self.config:
			...
			if len(v[1]) == 5:
				...
				if Is_Json == 1:
						...
						for url in self.Index_Url:
								request = Request(url,callback = self.parse_splash,meta=meta)
								request.meta['Index_Url'] = url
								request.meta['Max_Page'] = Max_Page
								request.meta['All_Detail_Page'] = All_Detail_Page
								request.meta['Signal_Detail_Page'] = Signal_Detail_Page
	...
	def parse_splash(self,response):
		#这边就是管你有没有，我都接收，在使用的时候判断，如果不存在，说明要直接到final_parse处(注：除了json页面)
		...
		if All_Detail_Page is None:
				...
				if "is_json" in All_Detail_Page.keys():
						print "我到了这里没有"
						...
						for i in range(1,2):
								try:
										...
										print Exception,":",e
								print "now the url is in json1 : %s"%url
								request = Request(url,callback = self.parse_json2)
	...
	def parse_json3(self,response):
		...
		depth = 0
		try:
				while depth < len(Signal_Detail_Page['index']):
						res_json = res_json.get(Signal_Detail_Page['index'][depth])
						depth += 1
		except Exception,e:
				print Exception,":",e
		urls = get_HeadUrl(response.url,self.name)	
		
		#print "now the res_json22222 is %s"%res_json
	...
	def parse_json4(self,response):
		...
		depth = 0
		length = len(Target_Detail_Page['index'])
		while depth < length - 1:
		...
		try:
				#这里我换成了get_letv_url来拿到全部的带有sid的url
				#detail_url = Relative_to_Absolute(Index_Url,detail_url)
				detail_url = get_letv_url2(detail_url)
		except Exception,e:
				print Exception,":",e
		for url in detail_url:
